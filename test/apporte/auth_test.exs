defmodule Apporte.AuthTest do
  ...
  describe "login/1" do
    test "success: it authenticates a Apporte.Accounts.User when valid params is given" do
      user_params = Factory.build(:user_validator)
      assert {:ok, %User{} = created_user} = Accounts.create_user(user_params)
    ...
    test "error: it returns an :unauthorized when given invalid params" do
      ...
      assert {:error, :unauthorized} = Auth.login(email, password)
    end
