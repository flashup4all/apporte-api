defmodule Apporte.Accounts.UserToken do
  @moduledoc false
  use Apporte.Schema
  import Ecto.Query
  alias Apporte.Repo
  ...
  def create_user_token(%Apporte.Accounts.User{} = user, context) do
  ...
  def deactivate_all(%Apporte.Accounts.User{} = user) do
    query =
      __MODULE__
      |> where([utk], utk.status == :active)
  ...
  def verify_email_token(%Apporte.Accounts.User{} = user, token, context) do
