defmodule ApporteWeb.SubscriptionController do
  use ApporteWeb, :controller

  alias Apporte.Billings
  alias Apporte.Billings.{Subscription}
  alias ApporteWeb.Validators.{CreateSubscription}
  ...
  def create(conn, params) do
    ...
    with {:ok, validated_params} <- CreateSubscription.cast_and_validate(params),
         {:ok, %Order{} = order} <- Billings.create_subscription(user, validated_params) do
      conn
      |> put_status(:created)
