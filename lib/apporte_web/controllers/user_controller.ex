defmodule ApporteWeb.UserController do
  use ApporteWeb, :controller

  alias Apporte.{Accounts, Auth, Repo}

  alias Apporte.Accounts.User
  alias ApporteWeb.Validators.{CreateUser, EmailValidator, ResetPassword}
  ...
  def register(conn, params) do
    with {:ok, validated_params} <- CreateUser.cast_and_validate(params),
         {:ok, %User{} = user} <- Accounts.create_user(validated_params) do
      conn
      |> put_status(:created)
  ...
  def create_staff(conn, params) do
    user = ApporteWeb.Auth.Guardian.Plug.current_resource(conn, [])

    with {:ok, validated_params} <- CreateUser.create_staff_cast_and_validate(params),
         {:ok, %User{} = user} <- Accounts.create_staff(user, validated_params) do
      conn
      |> put_status(:created)
      # |> put_resp_header("location", ~p"/api/users/#{user}")
      |> render(:show, user: user)
  ...
  def reset_password(conn, params) do
    ...
    # with {:ok, %ResetPassword{email: email, token: token, password: password}} <- ResetPassword.cast_and_validate(params),
